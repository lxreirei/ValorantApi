...
with open("proxy/proxy.txt", 'r', encoding='UTF-8')as f:
    proxies = "http://" + random.choice(f.readlines()).strip()
...
class RiotAuth:
    ...
    def __get_keys_from_access_token(
        self, key_attr_pairs: Sequence[Tuple[str, str]]) -> List[Tuple[str, Union[str, int, List, Dict, None]]]:
    ...
    def __set_tokens_from_uri(self, data: Dict) -> None:
        ...
        data = dict(parse_qsl(result))
        self.__update(extract_jwt=True, **data)
    async def reauthorize(self, load = None) -> bool:
        """
        ...
        if load:
            ...
            self._cookie_jar.update_cookies(cookies_dict)
        try:
    ...
    async def authorize(
        ...
        async with aiohttp.ClientSession(connector=conn, raise_for_status=True, cookie_jar=self._cookie_jar) as session:
            ...
            async with session.post("https://auth.riotgames.com/api/v1/authorization", json=body, headers=headers, proxy=proxies) as r:
